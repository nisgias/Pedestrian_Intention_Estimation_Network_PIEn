# train_v3_transfer.py
"""
Two-stage Transfer Learning: JAAD pretrain → PIE fine-tune
Includes PIE-only baseline for direct comparison.

Fixes vs previous version:
  Fix 1 — ReduceLROnPlateau patience 10 → 4. LR now actually drops before
           early stopping fires (model peaks at epoch 4-6, patience=10 was
           racing early stopping and always losing).
  Fix 3 — MultiTaskLoss now applies entropy regularization to BOTH:
             modality_weights  (2-way visual/kin final attention)
             visual_fuse_weights (160-token local+global attention)
           Previously only modality_weights was regularized, so the 160-token
           attention could collapse freely.
           New total loss:
             main + aux + entropy(modality) + entropy(visual_fuse)
           Both weighted by the same --entropy_weight flag.
"""
...
class MultiTaskLoss(nn.Module):
    def __init__(self, aux_weight: float = 0.1, entropy_weight: float = 0.05,
    ...
    def forward(self, outputs: dict, labels: torch.Tensor):
        ...
        # Fix 3: entropy on BOTH attention distributions
        ...
        if 'modality_weights' in outputs:
            ...
            ent_norm = ent / torch.log(torch.tensor(w.shape[-1], device=device, dtype=ent.dtype))
        ...
        # (b) 160-token visual fusion attention
        if 'visual_fuse_weights' in outputs:
            w2 = outputs['visual_fuse_weights']  # (B, T*S=160)
            ent2 = -(w2 * torch.log(w2 + 1e-8)).sum(dim=-1).mean()
            ent2_norm = ent2 / torch.log(torch.tensor(w2.shape[-1], device=device, dtype=ent2.dtype))
...
def run_train_epoch(model, loader, device, criterion, optimizer,
    ...
    running_loss = 0.0
    running_aux_kin = running_aux_local = running_aux_global = 0.0
    total = 0
...
def print_metrics(metrics, prefix=""):
    ...
    if 'auc_kin' in metrics:
        print(f"{prefix} | Branch AUC:  kin={metrics['auc_kin']:.3f}  "
              f"local={metrics['auc_local']:.3f}  global(conv3d)={metrics['auc_global']:.3f}")
    if 'w_visual' in metrics:
        print(f"{prefix} | Final Attn:  visual={metrics['w_visual']:.3f}  "
...
CSV_COLUMNS = [
    "stage", "epoch", "split", "loss", "auc", "f1", "acc", "precision", "recall",
    "auc_kin", "auc_local", "auc_global",
...
def log_csv(path, stage, epoch, split, metrics, lr=None):
    row = [
        stage, epoch, split,
...
def run_training_stage(
    ...
    epochs, use_amp, save_dir, csv_path,
    ...
    for epoch in range(1, epochs + 1):
        ...
        log_csv(csv_path, stage_name, epoch, "train", train_m, lr=current_lr)

        val_m = run_eval_epoch(model, val_loader, device, criterion, use_amp)
        print_metrics(val_m, f"[{stage_name}] Val  ")
        log_csv(csv_path, stage_name, epoch, "val", val_m, lr=current_lr)
    ...
    if best_state:
        ...
        log_csv(csv_path, stage_name, best_state["epoch"], "test", test_m)
...
def build_optimizer(model, lr, last_fc_l2, fixed_lr=False):
    fc_params = list(model.fc_out.parameters())
    if hasattr(model, "branch_logit_fusion"):
        fc_params += list(model.branch_logit_fusion.parameters())
    ...
    optimizer = torch.optim.RMSprop([
        ...
        {"params": fc_params,   "weight_decay": last_fc_l2},
    ], lr=lr)

    if fixed_lr:
        scheduler = None
    else:
        # Fix 1: patience 10 → 4 so LR drops before early stopping fires
...
def set_seed(seed: int):
    ...
    # Keep benchmark off for less run-to-run algorithm variation.
    torch.backends.cudnn.benchmark = False

    # Do not force full deterministic mode yet; it can slow training and break some ops.
...
def main():
    ...
    if not args.skip_transfer:
        ...
        if args.jaad_ckpt:
            ...
            model_s1    = PIPNetAlphaV3Final(dropout_p=args.dropout_p).to(device)
            criterion_s1 = MultiTaskLoss(args.aux_weight, args.entropy_weight, jaad_wpos)
            opt_s1, sch_s1 = build_optimizer(model_s1, args.jaad_lr,
                                              args.last_fc_l2, args.fixed_lr)
            ...
            _, jaad_ckpt = run_training_stage(
                ...
                args.jaad_epochs, use_amp, stage1_dir, csv_path,
        ...
        model_s2 = PIPNetAlphaV3Final(dropout_p=args.dropout_p).to(device)
        model_s2.load_state_dict(jaad_ckpt["model"])
        criterion_s2 = MultiTaskLoss(args.aux_weight, args.entropy_weight, pie_wpos)
        opt_s2, sch_s2 = build_optimizer(model_s2, args.pie_lr,
                                          args.last_fc_l2, args.fixed_lr)
        ...
        transfer_test_m, transfer_best_state = run_training_stage(
            ...
            args.pie_epochs, use_amp, stage2_dir, csv_path,
            early_stop_patience=args.early_stopping_patience,
        )
    ...
    if not args.skip_baseline:
        ...
        model_s3    = PIPNetAlphaV3Final(dropout_p=args.dropout_p).to(device)
        criterion_s3 = MultiTaskLoss(args.aux_weight, args.entropy_weight, pie_wpos)
        opt_s3, sch_s3 = build_optimizer(model_s3, args.baseline_lr,
                                          args.last_fc_l2, args.fixed_lr)
        ...
        baseline_test_m, _ = run_training_stage(
            ...
            args.baseline_epochs, use_amp, stage3_dir, csv_path,
            early_stop_patience=args.early_stopping_patience,
        )
