...
def make_loaders(
    ...
    train_ds = PIESeqDataset(
        data_root,
        split="train",
        mode="train",
        **common_args,
    )
    val_ds = PIESeqDataset(
        data_root,
        split="val",
        mode="eval",
        **common_args,
    )
    test_ds = PIESeqDataset(
        data_root,
        split="test",
        mode="eval",
        **common_args,
...
def run_train_epoch(
    ...
    running_loss = 0.0
    running_aux_kin = 0.0
    running_aux_local = 0.0
    running_aux_global = 0.0
    total = 0
    ...
    for batch in pbar:
        ...
        if "aux_kin" in outputs:
            all_probs_kin.append(
                torch.sigmoid(outputs["aux_kin"].squeeze(-1))
                .detach()
                .cpu()
                .numpy()
            )
            all_probs_local.append(
                torch.sigmoid(outputs["aux_local"].squeeze(-1))
                .detach()
                .cpu()
                .numpy()
            )
            all_probs_global.append(
                torch.sigmoid(outputs["aux_global"].squeeze(-1))
                .detach()
                .cpu()
                .numpy()
            )

            # Diagnostic visual AUC proxy:
            # z_visual is fused local+global.
            # This uses aux_local only for logging and does not affect training.
            if "z_visual" in outputs and hasattr(model, "aux_local"):
                with torch.no_grad():
                    visual_logit = model.aux_local(
                        outputs["z_visual"].detach()
                    ).squeeze(-1)

                    all_probs_visual.append(
                        torch.sigmoid(visual_logit)
                        .detach()
                        .cpu()
                        .numpy()
                    )
        ...
        if "modality_weights" in outputs:
            modality_weights_sum += (
                outputs["modality_weights"].detach().cpu().mean(0)
            )
            weight_count += 1
    ...
    if all_probs_visual:
        metrics["auc_visual"] = safe_auc(
            labels_np,
            np.concatenate(all_probs_visual),
        )
...
def run_eval_epoch(model, loader, device, criterion, use_amp=False):
    ...
    for batch in tqdm(loader, desc="Eval"):
        ...
        if "aux_kin" in outputs:
            ...
            # Diagnostic visual AUC proxy:
            # z_visual is fused local+global.
            # This uses aux_local only for logging.
            if "z_visual" in outputs and hasattr(model, "aux_local"):
                visual_logit = model.aux_local(outputs["z_visual"]).squeeze(-1)

                all_probs_visual.append(
                    torch.sigmoid(visual_logit).cpu().numpy()
                )
...
def log_csv(
    ...
    row = [
        ...
        (
            f"{metrics.get('visual_fuse_tokens', '')}"
            if "visual_fuse_tokens" in metrics
            else ""
        ),
...
def build_optimizer(
    ...
    base_params:
        Kinematic branch, kin attention, final attention, and other non-visual
        params. No weight decay.

    visual_params:
        Local visual branch, global branch, visual fusion attention, and
        visual/global auxiliary heads. Uses --visual_l2.

    final_fc_params:
        Final fc_out classifier. Uses --last_fc_l2.
    """
    base_params = []
    visual_params = []
    final_fc_params = []
    ...
    print(
        f"  visual weight_decay params: {sum(p.numel() for p in visual_params):,} "
        f"wd={visual_l2}"
    )
    print(
        f"  final_fc params:            {sum(p.numel() for p in final_fc_params):,} "
        f"wd={last_fc_l2}"
    )
    ...
    optimizer = torch.optim.RMSprop(
        [
            ...
            },
            {
                ...
                "weight_decay": visual_l2,
            },
            {
                "params": final_fc_params,
                "weight_decay": last_fc_l2,
            },
        ],
        lr=lr,
    )
...
def main():
    ...
    # ========================================================
    # S1/S2: JAAD pretrain → PIE fine-tune
    # ========================================================
    ...
    if not args.skip_transfer:
        ...
        if args.jaad_ckpt:
            ...
            model_s1 = PIPNetAlphaV4Final(
                dropout_p=args.dropout_p
            ).to(device)

            criterion_s1 = MultiTaskLoss(
                args.aux_weight,
                args.entropy_weight,
                jaad_wpos,
            )
        ...
        model_s2 = PIPNetAlphaV4Final(
            dropout_p=args.dropout_p
        ).to(device)

        model_s2.load_state_dict(jaad_ckpt["model"])

        criterion_s2 = MultiTaskLoss(
            args.aux_weight,
            args.entropy_weight,
            pie_wpos,
        )
    ...
    # ========================================================
    # S3: PIE-only baseline
    # ========================================================

    if not args.skip_baseline:
        stage3_dir = os.path.join(args.save_dir, "stage3_pie_baseline")

        model_s3 = PIPNetAlphaV4Final(
            dropout_p=args.dropout_p
        ).to(device)

        criterion_s3 = MultiTaskLoss(
            args.aux_weight,
            args.entropy_weight,
            pie_wpos,
        )
