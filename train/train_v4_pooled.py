#!/usr/bin/env python3
"""
Naive pooled JAAD+PIE training baseline for PIPNetAlphaV4Final.

Purpose:
  Train one V4 model on JAAD train + PIE train together, then early-stop and
  test on PIE only. This is the fair pooled counterpart of the V4 JAAD->PIE
  transfer experiment.

Important:
  JAAD has no real ego-speed. In the current preprocessing, JAAD speed is stored
  as zeros. Therefore this evaluates naive pooled training with zero-filled
...
def make_pooled_loaders(args, strict_len=True):
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="checkpoints_v4_pooled")
    ...
    print("=" * 90)
    print("V4 NAIVE POOLED TRAINING: JAAD train + PIE train -> PIE val/test")
    print("=" * 90)
    ...
    jaad_train, pie_train, pooled_train, train_loader, val_loader, test_loader = make_pooled_loaders(
        args, strict_len=strict_len
    )

    print(f"[INFO] JAAD train samples: {len(jaad_train)}")
    print(f"[INFO] PIE train samples:  {len(pie_train)}")
    print(f"[INFO] Pooled samples:     {len(pooled_train)}")
    print("[INFO] Early stopping and final test are PIE-only.")

    if args.wpos > 0:
        pooled_wpos = args.wpos
        print(f"[INFO] Using manual pos_weight={pooled_wpos:.4f}")
    else:
        pooled_wpos, pooled_pos, pooled_neg = compute_pos_weight_from_datasets(
            [jaad_train, pie_train]
        )
        print(
            f"[INFO] Pooled train class counts: "
            f"pos={pooled_pos}, neg={pooled_neg}, pos_weight={pooled_wpos:.4f}"
        )
    ...
    pooled_dir = os.path.join(args.save_dir, "stage0_pooled_jaad_pie")
    ...
    print("\n" + "=" * 90)
    print("POOLED FINAL PIE TEST RESULT")
    print("=" * 90)
