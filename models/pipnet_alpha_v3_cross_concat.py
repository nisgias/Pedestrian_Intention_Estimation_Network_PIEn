# models/pipnet_alpha_v3_cross_concat.py
"""
PIPNet-Alpha V3 Cross-Attention with Residual Fusion

Key Changes from Concat version:
1. Visual Fusion: Concat → Residual (z_local + z_cross)
   - z_cross already contains z_local info (z_local is the query)
   - Residual forces global branch to learn complementary info
   - Global branch cannot be ignored: z_cross always adds to z_local
   - Cleaner gradient signal: learn the DELTA not the full representation

Why Residual > Concat:
- Concat: model can set W_cross≈0, making z_cross invisible
- Residual: z_cross always adds to z_local, cannot be zeroed out
- Global branch is pressured to learn what appearance alone misses
"""
...
class GlobalContextBranch8x8(nn.Module):
    """
    Global context with 8×8 spatial preservation.
    
    Instead of pooling to 1×1 (loses WHERE), we keep 8×8 grid.
    This gives Cross-Attention 64 "slots" to search through.
...
class PIPNetAlphaV3CrossConcat(nn.Module):
    """
    PIPNet-Alpha V3 with Cross-Attention + Residual Fusion
    ...
    3. Global → GRU (8×8 spatial) → h_global [B, T, 256]

    4. Cross-Attention: z_local queries h_global → z_cross [B, 128]

    5. ★ RESIDUAL FUSION ★
       z_visual = LayerNorm(z_local + z_cross)  [B, 128]
       - z_cross already contains z_local info (z_local was the query)
       - Residual: global branch learns the DELTA on top of appearance
       - Cannot be ignored: z_cross always adds to z_local
    ...
    def __init__(
        ...
        # ============ GLOBAL CONTEXT BRANCH (8×8 spatial) ============
        self.global_branch = GlobalContextBranch8x8(
        ...
        # z_visual = LayerNorm(z_local + z_cross)
        # No extra parameters needed — cleaner and more principled
    ...
    def forward(self, batch: dict, return_aux: bool = False):
        ...
        # ============ 4. CROSS-ATTENTION ============
        # z_local is query → scene frames weighted by pedestrian appearance
        z_cross, alpha_cross = self.cross_attn(z_local, h_global)
        
        # ============ 5. VISUAL FUSION: RESIDUAL ============
        # z_cross already knows z_local (it was the query)
        # Residual: global branch learns what appearance alone misses
...
if __name__ == "__main__":
    model = PIPNetAlphaV3CrossConcat()
    ...
    print(f"\n✓ Residual fusion: global branch learns delta on top of appearance!")