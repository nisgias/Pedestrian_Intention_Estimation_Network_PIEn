# models/pipnet_alpha_v3_spatial_fix.py
"""
PIPNet-Alpha V3 — SPATIAL FIX EXPERIMENT

Changes from pipnet_alpha_v3_cross_concat.py:
1. GlobalContextBranch8x8 → GlobalContextBranchSpatial
   - Removes AdaptiveAvgPool2d(1,1) that killed spatial info
   - Keeps 4×4 spatial grid = 16 tokens per frame
   - Removes GRU (cross-attention handles temporal selection)
   - Output: [B, T*16, 256] instead of [B, T, 256]

2. CrossAttention: no code change needed
   - Already handles variable-length context
   - Now queries 160 tokens instead of 10

Everything else IDENTICAL to original for fair comparison.
"""
...
class CrossAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, context: torch.Tensor):
        # query: [B, D_q]
        # context: [B, N, D_k]  — N can be T*16=160 now!
        
        Q = self.W_q(query)        # [B, D_q]
        K = self.W_k(context)      # [B, N, D_q]
        V = self.W_v(context)      # [B, N, D_q]
        
        scores = torch.bmm(Q.unsqueeze(1), K.transpose(-1, -2)).squeeze(1)  # [B, N]
        scores = scores * self.scale
        
        alpha = F.softmax(scores, dim=-1)  # [B, N]  — now 160 spatial-temporal weights
...
# ============================================================
# ★ FIXED GLOBAL BRANCH — preserves 4×4 spatial grid ★
# ============================================================

class GlobalContextBranchSpatial(nn.Module):
    """
    Global context with 4×4 spatial preservation.
    
    WHAT CHANGED:
    - Old: 8×8 → pool to 1×1 → FC → GRU → [B, T, 256]  (10 temporal slots)
    - New: 8×8 → stride-2 → 4×4 → flatten → [B, T*16, 256]  (160 spatial-temporal tokens)
    
    WHY:
    - Cross-attention can now query WHERE: "crosswalk at position (2,3) in frame 7"
    - No GRU = fewer params = less overfitting on 821 samples
    - Cross-attention handles temporal selection naturally
    """
    def __init__(self, sem_num_classes=20, sem_embed_dim=16, hidden_dim=256):
        ...
        # ★ KEY CHANGE: 8×8 → 4×4, NO pooling to 1×1 ★
        self.combine_conv = nn.Sequential(
            nn.Conv2d(512, 256, kernel_size=3, stride=2, padding=1),  # 8×8 → 4×4
            nn.BatchNorm2d(256), nn.ReLU(inplace=True),
            nn.Conv2d(256, hidden_dim, kernel_size=1),  # channel adjust, stays 4×4
    ...
    def forward(self, sem_labels, cat_depth):
        ...
        # Combine: [B*T, 512, 8, 8] → [B*T, 256, 4, 4]
...
class PIPNetAlphaV3SpatialFix(nn.Module):
    """
    Same as PIPNetAlphaV3CrossConcat but with spatial fix.
    
    ONLY CHANGE: GlobalContextBranch8x8 → GlobalContextBranchSpatial
    Cross-attention context: [B, T, 256] → [B, T*16, 256]
    
    Everything else identical for fair A/B comparison.
    """
    def __init__(
        self,
        bbox_dim: int = 4,
        ...
        # ============ ★ FIXED GLOBAL BRANCH ★ ============
        self.global_branch = GlobalContextBranchSpatial(
            sem_num_classes=sem_num_classes,
            sem_embed_dim=sem_embed_dim,
            hidden_dim=256
        )
        
        # ============ CROSS-ATTENTION (unchanged code, new context size) ============
        self.cross_attn = CrossAttention(
            query_dim=branch_out_dim,
            key_dim=256,           # still 256
            out_dim=branch_out_dim,
            dropout_p=dropout_p
        )
        
        # ============ VISUAL FUSION: RESIDUAL (unchanged) ============
    ...
    def forward(self, batch: dict, return_aux: bool = False):
        ...
        # ============ 3. ★ GLOBAL: now returns spatial tokens ★ ============
        # Old: h_global = [B, T, 256]     → 10 temporal slots
        # New: h_global = [B, T*16, 256]  → 160 spatial-temporal tokens
        h_global = self.global_branch(sem_labels, cat_depth)
        
        # ============ 4. CROSS-ATTENTION (unchanged code) ============
        # z_local queries 160 spatial-temporal tokens
        # "Which scene locations are relevant to this pedestrian's appearance?"
        z_cross, alpha_cross = self.cross_attn(z_local, h_global)
        
        # ============ 5. VISUAL FUSION: RESIDUAL (unchanged) ============
        z_visual = self.norm_visual(z_local + z_cross)
        
        # ============ 6. FINAL ATTENTION (unchanged) ============
...
if __name__ == "__main__":
    ...
    print(f"  → cross-attn now attends over {out['cross_attn_weights'].shape[1]} spatial-temporal tokens!")
    print(f"\n✓ Spatial fix: global branch preserves 4×4 grid!")
