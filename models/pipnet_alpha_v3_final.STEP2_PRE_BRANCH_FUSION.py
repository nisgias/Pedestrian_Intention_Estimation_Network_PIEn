...
★ Step-2 regularization added (visual branches only):
  - LocalVisualBranch: Dropout(0.3) between the two per-modality GRUs
    and the fuse GRU.
  - GlobalContextBranchConv3D: Dropout3d(0.2) between Conv3D stages on
    both semantic and depth streams.
...
class LocalVisualBranch(nn.Module):
    def __init__(self, cnn_dim=512, hidden_dim=256, dropout_p=0.3):
        ...
        # ★ Step-2 regularization: dropout between GRUs
...
class GlobalContextBranchConv3D(nn.Module):
    ...
    ★ Step-2 regularization: Dropout3d(0.2) between Conv3D stages on both
    semantic and depth streams (6 insertions total, symmetric).
    """
...
class PIPNetAlphaV3Final(nn.Module):
    ...
    Changes from proven 0.8445 AUC model:
      - Added nn.Dropout(0.5) before fc_out                        (previous step)
      - ★ Dropout(0.3) between GRUs in LocalVisualBranch            (step 2)
      - ★ Dropout3d(0.2) between Conv3D stages in GlobalContext     (step 2)
    """
    def __init__(
        ...
        local_dropout_p=0.3, global_dropout_p=0.2,  # ★ new knobs, defaulted
    ):
        super().__init__()
        ...
        self.fc_drop = nn.Dropout(dropout_p)  # (previous step)
