...
def pick_window(frame_idx_ds, keep_idx, seq_len, target_end):
    if frame_idx_ds.size < seq_len:
        return None
    t_end = int(np.searchsorted(frame_idx_ds, target_end, side="right") - 1)
    if t_end < 0:
        return None
    t_start = t_end - (seq_len - 1)
    if t_start < 0:
        return None
    return t_start, t_end, int(keep_idx[t_start]), int(keep_idx[t_end])
...
def main():
    ap = argparse.ArgumentParser("Step 1: Kinematics (bbox, pose, speed)")
    ...
    ap.add_argument("--trainval_tte_sec", type=float, default=1.0)
    ap.add_argument("--test_tte_sec", type=float, default=1.0)
    ...
    print("=" * 70)
    print("Step 1: KINEMATICS (bbox, pose, speed)")
    print("=" * 70)
    ...
    try:
        for split in splits:
            ...
            tte_sec = (args.test_tte_sec if split == "test" else args.trainval_tte_sec)
            raw_offset = int(round(tte_sec * args.raw_fps))  # EXACT horizon in raw frames
            ...
            for idx in range(n):
                ...
                pid = str(pid_all[idx]) if pid_all and idx < len(pid_all) else f"unk_{idx}"
                ...
                # Sync
                vc.infer_offset(set_id, video_name, img_paths, frame_idx, verbose=(args.debug > 0 and written < 3))
                ...
                target_end = (event_frame if cross_label == 1 else pseudo_frame) - raw_offset
                
                # Window
                win = pick_window(frame_idx_ds, keep_idx, args.seq_len, target_end)
                if win is None:
                    continue
                t_start_ds, t_end_ds, t_start_full, t_end_full = win
                
                # Irrelevant check
                if not args.keep_irrelevant:
                    _, irr = _cross_masks(cross)
                    if irr[keep_idx[t_start_ds:t_end_ds + 1]].any():
                        continue
                
                # Slice
                frame_idx_win = frame_idx_ds[t_start_ds:t_end_ds + 1]
                boxes_win = boxes[keep_idx][t_start_ds:t_end_ds + 1]
                intent_win = intent[keep_idx][t_start_ds:t_end_ds + 1]
                img_paths_win = [img_paths[k] for k in keep_idx[t_start_ds:t_end_ds + 1]]
                
                if np.any(np.diff(frame_idx_win) <= 0):
                    continue
                
                # TTE
                ref_frame = event_frame if cross_label == 1 else pseudo_frame
                tte_actual = float(ref_frame - frame_idx_win[-1]) / args.raw_fps
                tte_target = float(tte_sec)

                
                if cross_label == 1 and tte_actual <= 0:
                    continue
                ...
                try:
                    ...
                    if args.debug > 0 and written < 3:
                        ...
                        print(f"[CHECK] frame WxH={w}x{h} | box x in [{mnx:.1f},{mx:.1f}] | box y in [{mny:.1f},{my:.1f}]")
                        if mx > w + 2 or my > h + 2 or mnx < -2 or mny < -2:
                ...
                for fr, (x1, y1, x2, y2) in zip(frames, boxes_win):
                    ...
                    bbox_norm.append([x1 / w, y1 / h, x2 / w, y2 / h])
                ...
                if speed is not None:
                    speed_win = speed[keep_idx][t_start_ds:t_end_ds + 1].reshape(-1, 1)
                else:
                ...
                meta = {
                    ...
                    "intention_prob_full": intent.tolist(),  # full track (trimmed to T_full)
                }
            ...
            print(f"[OK] {split}: {written} samples (pos={pos}, neg={neg})")
