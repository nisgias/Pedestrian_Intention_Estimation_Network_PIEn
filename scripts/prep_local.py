#!/usr/bin/env python3
"""
Step 2: Local Features - VGG19 CNN + RAFT Optical Flow
Updates existing NPZ files created by prep_kinematics.py

Example:
  python scripts/prep_local.py \
    --npz_root /workspace/PIE_PREP_OUT/ETC0_5 \
    --pie_root /data/PIE \
...
class VideoCache:
    ...
    def read_frame(self, set_id, video_name, frame_idx, offset=0):
        cap = self.get_cap(set_id, video_name)
        idx = int(frame_idx) + int(offset)
        if idx < 0:
            raise RuntimeError(f"Invalid frame idx={idx} (frame_idx={frame_idx}, offset={offset})")

        cap.set(cv2.CAP_PROP_POS_FRAMES, idx)
        ok, frame = cap.read()
        if not ok or frame is None:
            raise RuntimeError(f"Failed to read frame {idx} from {set_id}/{video_name}")
        return frame
...
def main():
    ap = argparse.ArgumentParser("Step 2: Local Features (VGG + RAFT Flow)")
    ...
    try:
        for split in splits:
            ...
            for npz_path in npz_files:
                ...
                set_id = str(data["set_id"][0]) if getattr(data["set_id"], "ndim", 1) != 0 else str(data["set_id"])
                ...
                try:
                    frames = [vc.read_frame(set_id, video_name, int(fi), mp4_off) for fi in frame_idx]
