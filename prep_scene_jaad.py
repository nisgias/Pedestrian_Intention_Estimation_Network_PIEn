#!/usr/bin/env python3
"""
Step 3: Scene Features using Mask2Former (Cityscapes Panoptic) + ManyDepth  (JAAD version)
...
Cache format (JAAD - flat, no set_id nesting):
  cache_root/video_XXXX/{semantic,instance,inst_label}/00000.png

NPZ keys kept compatible:
  sem_labels, cat_depth, dvis_inst_raw, dvis_inst

Run examples:
1) generate (produces per-frame cache PNGs):
python scripts/prep_scene_mask2former_cityscapes_jaad.py --mode generate \
  --jaad_root /data/JAAD \
  --cache_root /workspace/project/m2f_cache_jaad \
  --npz_root /workspace/JAAD_PREP_OUT \
  --splits train,val,test \
  --out_h 384 --out_w 672 \
  --force

2) preprocess (fills sem_labels, cat_depth, dvis_inst_raw, dvis_inst):
python scripts/prep_scene_mask2former_cityscapes_jaad.py --mode preprocess \
  --jaad_root /data/JAAD \
  --cache_root /workspace/project/m2f_cache_jaad \
  --npz_root /workspace/JAAD_PREP_OUT \
...
# ============================================================
# Collect Required Frames from NPZ files (JAAD: flat, no set_id in keys)
# ============================================================
...
def collect_required_frames(
    ...
    Returns:
        required: Dict mapping "video_name" -> set of frame indices
        offsets:  Dict mapping "video_name" -> mp4_png_offset
    """
        ...
        for npz_path in files:
            try:
                ...
                with np.load(npz_path, allow_pickle=True) as data:
                    ...
                    required[video].update(frames)

                    off = int(data.get("mp4_png_offset", 0))
                    off_counts[video][off] += 1
            except Exception as e:
...
def init_manydepth(weights_path: str = "/workspace/models/manydepth/KITTI_HR"):
    ...
    jaad_ratio = 1920 / 1080  # JAAD also uses 1920x1080
    if abs(feed_ratio - jaad_ratio) > 0.5:
        print(f"[WARN] ManyDepth aspect ratio ({feed_ratio:.2f}) differs significantly from JAAD ({jaad_ratio:.2f})")
...
# ============================================================
# Video Cache  (JAAD: single-open, OOM-safe, flat directory)
# ============================================================
...
class VideoCache:
    ...
    def read(self, video_name: str, idx: int, off: int = 0) -> np.ndarray:
        cap = self.get_cap(video_name)
        target = idx + off
        if target < 0:
            raise RuntimeError(f"Invalid frame idx={target} (idx={idx}, off={off})")
        cap.set(cv2.CAP_PROP_POS_FRAMES, target)
        ok, frame = cap.read()
        if not ok or frame is None:
            raise RuntimeError(f"Failed to read frame {target} from {video_name}")
        return frame

    def reads(self, video_name: str, idxs: List[int], off: int = 0) -> List[np.ndarray]:
        return [self.read(video_name, i, off) for i in idxs]
...
# ============================================================
# Generate Cache (only needed frames)  — JAAD version
# ============================================================
...
def generate(args):
    ...
    processed_total = 0
    try:
        for video_name in tqdm(sorted(required.keys()), desc="Videos"):
            # JAAD: flat cache — cache_root/video_name/{semantic,instance,inst_label}/
            cdir = cache / video_name

            frame_idxs = sorted(required[video_name])
            off = int(offsets.get(video_name, 0))
            ...
            for idx in tqdm(frame_idxs, desc=f"{video_name}", leave=False):
                try:
                    frame = vc.read(video_name, idx, off=off)
                    sem, inst, inst_lbl = m2f.run_single(frame, out_size)
                    save_frame_cache(cdir, sem, inst, inst_lbl, idx)
                    processed_total += 1
                except Exception as e:
                    print(f"[ERR] {video_name} frame {idx} (off={off}): {e}")
    finally:
        vc.release()
    ...
    try:
        for split in splits:
            ...
            for npz in tqdm(files, desc=split):
                ...
                # Load cache — JAAD: flat, cache_root/video_name/
                sem_city, inst, inst_lbl = load_cache(cache / video, fidx, out_size)
                if sem_city is None:
                    cache_miss += 1
                    continue
                ...
                try:
                    frames = vc.reads(video, fidx, off)
                except Exception as e:
...
def main():
    ...
    p.add_argument("--jaad_root", required=True)
    ...
    # Output resolution - RECTANGULAR (JAAD is 1920x1080, default to half: 960x540 to keep 16:9)
