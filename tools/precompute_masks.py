#!/usr/bin/env python3
"""
Offline mask precomputation – cache version (FIXED)
"""
...
def process_file(npz_path: str, root: str, cache_dir: str, target_size: int, overwrite: bool) -> str:
    try:
        ...
        print(f"\n  ERROR on {os.path.basename(npz_path)}: {e}", flush=True)
        return "ERROR"


def main():
    p = argparse.ArgumentParser()
    p.add_argument("root", type=str, help="Dataset root, e.g. /Datasets/PIE_PREP_OUT")
    p.add_argument("--cache_dir", type=str, default="/workspace/project/masks_cache",
                   help="Where to store precomputed mask files")
    ...
    for split in args.splits:
        ...
        print(f"  -> OK: {counts['OK']}, SKIPPED: {counts['SKIPPED']}, "
              f"NO_SEM: {counts['NO_SEM']}, ERROR: {counts['ERROR']}\n")
