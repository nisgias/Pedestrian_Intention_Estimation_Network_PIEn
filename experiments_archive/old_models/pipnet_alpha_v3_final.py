...
"""
PIPNet-Alpha V3 — Step-2 + 4 fixes
...
Unchanged from Step-2:
  ...
  - All Conv3D channels, GRU sizes, parameter count (~7.66M + tiny global_attn)
"""
...
class GlobalContextBranchConv3D(nn.Module):
    """
    Conv3D channels: 64→128→256. Step-2 Dropout3d(0.2) between stages.
    """
    def __init__(self, sem_num_classes=20, sem_embed_dim=16, hidden_dim=256,
                 target_size=64, dropout_p=0.2):
        super().__init__()
...
class PIPNetAlphaV3Final(nn.Module):
    def __init__(
        ...
        local_dropout_p=0.3, global_dropout_p=0.2,
    ):
        super().__init__()
        ...
        # --- Visual fusion: local + 160 global tokens ---
        ...
        # Fix 2: proper attention head for global aux (same quality as kin/local)
    ...
    def forward(self, batch: dict, return_aux: bool = False):
        ...
        # --- Global context ---
        h_global = self.global_branch(sem_labels, cat_depth)   # (B, T, S, 256)
        S = h_global.shape[2]
        h_global_flat = h_global.reshape(B, T * S, 256)        # (B, T*S, 256)

        # --- Visual fusion ---
        z_local_expanded = z_local.unsqueeze(1).expand(-1, T * S, -1)

        if getattr(self, '_ablate_global', False):
            # Clean global ablation: zero only the global tokens.
            # visual_fuse_attn still runs with the same architecture and
            # local pathway intact. Only global information is removed.
            h_global_for_fusion = torch.zeros_like(h_global_flat)
        else:
            h_global_for_fusion = h_global_flat

        h_fused = torch.cat([z_local_expanded, h_global_for_fusion], dim=-1)  # (B, T*S, 384)
        ...
        if return_aux:
            # Fix 2: global aux via proper attention over all T*S global tokens
            z_global_att, _ = self.global_attn(h_global_flat, use_mean_query=True)
            ...
            return {
                ...
                'aux_global': self.aux_global(z_global_att),  # Fix 2
                'modality_weights': alpha_final,              # (B, 2)
                'visual_fuse_weights': alpha_visual,          # (B, T*S=160) — Fix 3
...
if __name__ == "__main__":
    ...
    print(f"aux_kin:             {out['aux_kin'].shape}")
    print(f"aux_global:          {out['aux_global'].shape}")
    print(f"modality_weights:    {out['modality_weights'].shape}")
