# train_v3_cross_concat.py
"""
Training script for PIPNet-Alpha V3 Cross-Attention + Residual Fusion

Key difference: Visual fusion uses RESIDUAL (z_local + z_cross)
...
from models.pipnet_alpha_v3_option_c_spatial import PIPNetAlphaV3OptionCSpatial
...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, outputs: dict, labels: torch.Tensor):
        ...
        # Entropy on final attention (visual vs kin)
...
def print_metrics(metrics, prefix=""):
    ...
    if 'w_visual' in metrics:
        ...
        print(f"{prefix} | Visual Fusion: RESIDUAL (z_local + z_cross)")


def main():
    parser = argparse.ArgumentParser(description="Train PIPNet-Alpha V3 Cross + Residual")
    ...
    parser.add_argument("--save_dir", type=str, default="checkpoints_v3_cross_residual")
    ...
    print("=" * 80)
    print("PIPNet-Alpha V3 Option C Spatial")
    print("=" * 80)
    print(f"Device: {device}")
    print(f"Key Change: Spatial GRU tokens + Joint Attention fusion")
    print(f"Global Branch: 8×8 spatial preservation")
    ...
    print("\nBuilding model...")
    model = PIPNetAlphaV3OptionCSpatial(dropout_p=args.dropout_p).to(device)
    ...
    print("Watch: global(cross) AUC - residual forces it to learn complementary info!")
    print("=" * 80)
    ...
    if best_state:
        ...
        if 'auc_kin' in test_m:
            ...
            print(f"  Global (Cross): AUC={test_m['auc_global']:.4f}")
        
        print(f"\nFinal Attention (visual vs kin):")
        if 'w_visual' in test_m:
            print(f"  Visual: {test_m['w_visual']:.3f}  Kin: {test_m['w_kin']:.3f}")
        print(f"  Visual Fusion: RESIDUAL (z_local + z_cross)")
        print("=" * 80)
